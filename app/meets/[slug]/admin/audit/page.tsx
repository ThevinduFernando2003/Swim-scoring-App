...
export default async function MeetAuditPage({
  params,
}: {
  ...
  const { data: uploads } = eventIds.length
    ? await supabase
        .from("uploads")
        .select(
          "id, event_id, file_path, uploaded_by, uploaded_at, confirmed, events(name, day, event_number, gender)",
        )
              ...
              return (
                <tr key={upload.id} className="border-t border-white/10">
                  ...
                  <td className="px-4 py-3">
                    {event ? (
                      <Link
                        href={`/meets/${meet.slug}/admin/events/${upload.event_id}`}
